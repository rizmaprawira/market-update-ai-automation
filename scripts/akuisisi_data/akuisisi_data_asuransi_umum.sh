...
if [[ "$FLAG_SKIP_KEY_METRIC" != "true" && "$FLAG_DRY_RUN" != "true" && "$FLAG_DISCOVER_ONLY" != "true" ]]; then
  ...
  DATABASE_CSV="${PERIOD_DIR}/database_asuransi_umum_${TAHUN}_${BULAN}.csv"

  mapfile -t ALL_METRIC_CSVS < <(find "${PERIOD_DIR}/asuransi_umum" -name "*_key_metric_${TAHUN}_${BULAN}.csv" 2>/dev/null | sort)
