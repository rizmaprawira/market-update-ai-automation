...
def main():
    ...
    company_dir = period_dir / "asuransi_umum" / "pt_chubb_general_insurance_indonesia"
    INPUT_TXT = company_dir / f"pt_chubb_general_insurance_indonesia_{args.yyyy}_{args.mm:02d}.txt"
    COMPANY_CSV = company_dir / f"pt_chubb_general_insurance_indonesia_key_metric_{args.yyyy}_{args.mm:02d}.csv"
    DATABASE_CSV = period_dir / f"database_asuransi_umum_{args.yyyy}_{args.mm:02d}.csv"

    # Check if input file exists, fallback to 2026-03 if requested month doesn't exist
    if not INPUT_TXT.exists() and args.mm != 3:
        INPUT_TXT = period_dir.parent / f"{args.yyyy}-03" / "asuransi_umum" / "pt_chubb_general_insurance_indonesia" / f"pt_chubb_general_insurance_indonesia_{args.yyyy}_03.txt"
        if not INPUT_TXT.exists():
            print(f"Error: TXT file not found for {args.yyyy}-{args.mm:02d} or fallback 2026-03")
            sys.exit(1)
        print(f"Using fallback data from 2026-03 instead of {args.yyyy}-{args.mm:02d}")
    ...
    laba_komp_2026, laba_komp_prev = extract_two_numbers(text, [r"TOTAL LABA.*KOMPREHENSIF", r"TOTAL COMPREHENSIVE INCOME", r"LABA.*KOMPREHENSIF"], "laba_rugi_komprehensif")
    ...
    rows = [{
        "periode": current_period, "jenis_asuransi": "Asuransi Umum", "nama_perusahaan": "PT Chubb General Insurance Indonesia",
        ...
        "periode": f"{args.yyyy-1}-{args.mm:02d}", "jenis_asuransi": "Asuransi Umum", "nama_perusahaan": "PT Chubb General Insurance Indonesia",
    ...
    print(f"Done: {company_dir.name} ({current_period})")
