"""Download financial reports for PT Asuransi Jasa Tania Tbk.."""
import argparse
import logging
import sys
import time
...
LOGGER = logging.getLogger("download_pt_asuransi_jasa_tania_tbk")
SOURCE_URL = "https://jastan.co.id/info/reports/finance"
STORAGE_BASE_URL = "https://jastan.co.id/storage/img/reports"
COMPANY_ID = "pt_asuransi_jasa_tania_tbk"
COMPANY_NAME = "PT Asuransi Jasa Tania Tbk."
CATEGORY = "asuransi_umum"

def discover_storage_candidates(session, html, year, month, timeout=30):
    """Try to find PDF in Jasa Tania storage folder with various naming patterns."""
    month_names_en = {
        ...
        for button in soup.find_all("button"):
            ...
            if "window.open" in onclick and ".pdf" in onclick:
                ...
                if match:
                    url = match.group(1)
                    url_lower = url.lower()
                    if str(year) in url and (month_en.lower() in url_lower or month_id.lower() in url_lower):
                        text = button.find_parent().find_previous("h5", class_="card-title")
                        text_str = text.get_text(strip=True) if text else "Report"
                        return {"url": url, "text": text_str[:60]}
    except Exception as e:
        LOGGER.debug(f"Failed to extract onclick PDF links: {e}")

    month_en = month_names_en[month]
    month_id = month_names_id[month]

    patterns = [
        f"Laporan%20Keuangan%20Website%20{month_en}%20{year}.pdf",
        f"Laporan%20Keuangan%20Website%20{month_id}%20{year}.pdf",
        f"Laporan%20Keuangan%20{month_en}%20{year}.pdf",
        f"Laporan%20Keuangan%20{month_id}%20{year}.pdf",
    ]

    for pattern in patterns:
        test_url = f"{STORAGE_BASE_URL}/{pattern}"
        try:
            resp = session.head(test_url, timeout=timeout, allow_redirects=True)
            if resp.status_code == 200:
                return {"url": test_url, "text": pattern}
        except Exception:
            pass

    raise Exception("No storage candidates found")
...
def main():
    ...
    if not candidates:
        try:
            candidate = discover_storage_candidates(session, html, args.year, args.month, timeout=args.timeout)
            candidates = [PDFCandidate(url=candidate["url"], text=candidate["text"], score=70, discovered_url=SOURCE_URL)]
            LOGGER.info(f"Discovered candidate via storage folder: {candidate['url']}")
        except Exception as e:
            LOGGER.debug(f"Storage discovery failed: {e}")
