"""Download financial reports for PT Asuransi Bina Dana Arta Tbk. (OONA Ins.)."""
import argparse
import logging
import sys
import time
...
LOGGER = logging.getLogger("download_pt_asuransi_bina_dana_arta_tbk_oona_ins")
SOURCE_URL = "https://myoona.id/tentang-kami/tata-kelola-perusahaan/"
COMPANY_ID = "pt_asuransi_bina_dana_arta_tbk_oona_ins"
COMPANY_NAME = "PT Asuransi Bina Dana Arta Tbk. (OONA Ins.)"
...
def discover_oona_exact_period_pdf(session, year, month, timeout):
    month_name = MONTH_EN[month]
    base = "https://myoona.id/content/dam/oona/content-fragments/id/id/about-us-corporate/laporan-keuangan/bulanan"
    filename = quote(f"Financial Statement {month_name} {year}.pdf")
    url = f"{base}/{year}/{filename}"
    response = session.get(url, timeout=timeout)
    response.raise_for_status()
    content_type = (response.headers.get("content-type") or "").lower()
    if "pdf" not in content_type and not response.content.startswith(b"%PDF-"):
        raise RuntimeError("OONA direct URL did not return a PDF")
    return url
...
def main():
    ...
    if not candidates:
        try:
            direct_url = discover_oona_exact_period_pdf(session, args.year, args.month, args.timeout)
            candidates = [type("C", (), {"url": direct_url, "text": direct_url, "score": 1000})()]
            LOGGER.info("Found exact period OONA monthly PDF via deterministic URL pattern")
        except Exception as e:
            LOGGER.warning(f"OONA direct monthly URL discovery failed: {e}")
    if not candidates:
        try:
