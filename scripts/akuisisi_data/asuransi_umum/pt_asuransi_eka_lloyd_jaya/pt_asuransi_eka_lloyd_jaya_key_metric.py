...
def main():
    ...
    company_dir = period_dir / "asuransi_umum" / "pt_asuransi_eka_lloyd_jaya"
    INPUT_TXT = company_dir / f"pt_asuransi_eka_lloyd_jaya_{args.yyyy}_{args.mm:02d}.txt"
    COMPANY_CSV = company_dir / f"pt_asuransi_eka_lloyd_jaya_key_metric_{args.yyyy}_{args.mm:02d}.csv"
    ...
    aset_2026, aset_prev = extract_two_numbers(text, "35 Total Aset (21 + 34)")
    ekuitas_2026, ekuitas_prev = extract_two_numbers(text, "20 Total Ekuitas (16 s.d. 19)")
    ...
    premi_neto_2026, premi_neto_prev = extract_two_numbers(text, "9 Jumlah Pendapatan Premi, Neto")
    hasil_uw_2026, hasil_uw_prev = extract_two_numbers(text, "16 HASIL UNDERWRITING")
    laba_komp_2026, laba_komp_prev = extract_two_numbers(text, "TOTAL LABA KOMPREHENSIF")
    ...
    rows = [{
        "periode": current_period, "jenis_asuransi": "Asuransi Umum", "nama_perusahaan": "PT Asuransi Eka Lloyd Jaya",
        ...
        "periode": f"{args.yyyy-1}-{args.mm:02d}", "jenis_asuransi": "Asuransi Umum", "nama_perusahaan": "PT Asuransi Eka Lloyd Jaya",
