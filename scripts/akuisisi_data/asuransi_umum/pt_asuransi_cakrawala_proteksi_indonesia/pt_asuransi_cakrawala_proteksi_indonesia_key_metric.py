...
def main():
    parser = argparse.ArgumentParser(description="Extract Cakrawala Proteksi metrics from TXT file")
    ...
    company_dir = period_dir / "asuransi_umum" / "pt_asuransi_cakrawala_proteksi_indonesia"
    INPUT_TXT = company_dir / f"pt_asuransi_cakrawala_proteksi_indonesia_{args.yyyy}_{args.mm:02d}.txt"
    COMPANY_CSV = company_dir / f"pt_asuransi_cakrawala_proteksi_indonesia_key_metric_{args.yyyy}_{args.mm:02d}.csv"
    ...
    aset_2026, aset_prev = extract_two_numbers(text, "21 JUMLAH ASET")
    ekuitas_2026, ekuitas_prev = extract_two_numbers(text, "19 JUMLAH EKUITAS (15 s.d. 18)")
    pend_premi_2026, pend_premi_prev = extract_two_numbers(text, "6 Jumlah Premi Bruto (3+4-5)")
    ...
    laba_komp_2026, laba_komp_prev = extract_two_numbers(text, "40 TOTAL LABA (RUGI) KOMPREHENSIF")
