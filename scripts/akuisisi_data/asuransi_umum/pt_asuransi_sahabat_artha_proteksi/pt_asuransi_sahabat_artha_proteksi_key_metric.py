...
def main():
    ...
    company_dir = period_dir / "asuransi_umum" / "pt_asuransi_sahabat_artha_proteksi"
    INPUT_TXT = company_dir / f"pt_asuransi_sahabat_artha_proteksi_{args.yyyy}_{args.mm:02d}.txt"
    COMPANY_CSV = company_dir / f"pt_asuransi_sahabat_artha_proteksi_key_metric_{args.yyyy}_{args.mm:02d}.csv"
    ...
    aset_2026, aset_prev = extract_two_numbers(text, "23 Jumlah Aset")
    ...
    premi_reasu_2026, premi_reasu_prev = extract_two_numbers(text, "5 Jumlah Premi Reas")
    premi_neto_2026, premi_neto_prev = extract_two_numbers(text, "Premi Neto (3-5)")
    hasil_uw_2026, hasil_uw_prev = extract_two_numbers(text, "16 HASIL UNDERWRITING")
    laba_komp_2026, laba_komp_prev = extract_two_numbers(text, "TOTAL LABA (RUGI) KOMPREHENSIF (24+25)")
    ...
    rows = [{
        "periode": current_period, "jenis_asuransi": "Asuransi Umum", "nama_perusahaan": "PT Asuransi Sahabat Artha Proteksi",
        ...
        "periode": f"{args.yyyy-1}-{args.mm:02d}", "jenis_asuransi": "Asuransi Umum", "nama_perusahaan": "PT Asuransi Sahabat Artha Proteksi",
