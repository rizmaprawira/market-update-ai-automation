"""Download financial reports for PT Arthagraha General Insurance."""
import argparse
import logging
import sys
import time
...
from _downloader_base import (
    ...
    discover_download_candidate, matches_target_period, current_timestamp,
    sync_playwright, validate_pdf
)

LOGGER = logging.getLogger("download_pt_arthagraha_general_insurance")
SOURCE_URL = "https://www.aggi.co.id/id/financialhighlight"
COMPANY_ID = "pt_arthagraha_general_insurance"
COMPANY_NAME = "PT Arthagraha General Insurance"
        ...
        try:
            with page.expect_download(timeout=timeout * 1000) as download_info:
                try:
                    ...
                    # Chromium often raises "Download is starting" while still emitting download event.
                    if "Download is starting" not in str(nav_exc):
                        raise
            download = download_info.value
            tmp_path = output_path.parent / f".{output_path.name}.part"
            download.save_as(str(tmp_path))
            data = tmp_path.read_bytes()
            if not validate_pdf(data):
                tmp_path.unlink(missing_ok=True)
                raise RuntimeError("browser download is not a valid PDF")
...
def main():
    ...
    candidates = [
        ...
        if matches_target_period(f"{c.text} {c.url}", args.year, args.month)
    ]
    ...
    try:
        ...
        try:
            http_status, file_size = download_pdf_via_browser(
                selected_candidate.url, output_pdf, timeout=args.timeout
            )
            success = True
            reason = f"downloaded via browser fallback (http_status={http_status}, bytes={file_size})"
            status = "downloaded"
        except Exception as browser_error:
            success = False
            reason = f"failed to download PDF: {e}; browser fallback failed: {browser_error}"
            status = "error"
