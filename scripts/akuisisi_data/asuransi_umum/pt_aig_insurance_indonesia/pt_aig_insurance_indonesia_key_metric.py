...
def main():
    parser = argparse.ArgumentParser(description="Extract AIG metrics from TXT file")
    ...
    company_dir = period_dir / "asuransi_umum" / "pt_aig_insurance_indonesia"
    INPUT_TXT = company_dir / f"pt_aig_insurance_indonesia_{args.yyyy}_{args.mm:02d}.txt"
    COMPANY_CSV = company_dir / f"pt_aig_insurance_indonesia_key_metric_{args.yyyy}_{args.mm:02d}.csv"
    ...
    company = "PT AIG Insurance Indonesia"
    jenis = "Asuransi Umum"

    # Keywords dari laporan keuangan AIG 2026-04
    ...
    pend_premi_2026, pend_premi_prev = extract_two_numbers(text, "3 Jumlah Pendapatan Premi")
    premi_bruto_2026, premi_bruto_prev = extract_two_numbers(text, "4 Jumlah Premi Bruto")
    premi_reasu_2026, premi_reasu_prev = extract_two_numbers(text, "6 Jumlah Premi Reasuransi")
    premi_neto_2026, premi_neto_prev = extract_two_numbers(text, "7 Jumlah Premi Neto")
    hasil_uw_2026, hasil_uw_prev = extract_two_numbers(text, "17 HASIL UNDERWRITING")
    laba_komp_2026, laba_komp_prev = extract_two_numbers(text, "27 TOTAL LABA RUGI DAN PENGHASILAN KOMPREHENSIF LAIN")
