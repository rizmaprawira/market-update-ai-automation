...
def main():
    parser = argparse.ArgumentParser(description="Extract Nasional Indonesia metrics from TXT file")
    ...
    company_dir = period_dir / "reasuransi" / "pt_reasuransi_nasional_indonesia"
    INPUT_TXT = company_dir / f"pt_reasuransi_nasional_indonesia_{args.yyyy}_{args.mm:02d}.txt"
    COMPANY_CSV = company_dir / f"pt_reasuransi_nasional_indonesia_key_metric_{args.yyyy}_{args.mm:02d}.csv"
    ...
    aset_2026, aset_prev = extract_two_numbers(text, "JUMLAH ASET")
    ekuitas_2026, ekuitas_prev = extract_two_numbers(text, "Jumlah Ekuitas")
    premi_tl_2026, premi_tl_2025 = extract_two_numbers(text, "Premi Penutupan Tidak Langsung")
    premi_bruto_2026, premi_bruto_2025 = extract_two_numbers(text, "Jumlah Premi Bruto")
    pend_premi_2026, pend_premi_2025 = extract_two_numbers(text, "Jumlah Pendapatan Premi")
    hasil_uw_2026, hasil_uw_2025 = extract_two_numbers(text, "HASIL UNDERWRITING")
    laba_komp_2026, laba_komp_2025 = extract_two_numbers(text, "LABA (RUGI) KOMPREHENSIF")
